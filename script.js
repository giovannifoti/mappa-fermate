document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('darkToggle').addEventListener('click', () => {
    ...
    if (isDark) { map.removeLayer(lightLayer); map.addLayer(darkLayer); }
  ...
  function toggleFavorite(id) { const str=id.toString(); const idx=favorites.indexOf(str); if(idx>-1) favorites.splice(idx,1); else favorites.push(str); localStorage.setItem('favorites', JSON.stringify(favorites)); }
  ...
  fetch('./stops_fixed.json')
    ...
    .then(data=>{
      ...
      stops = data.filter(s=>s&&s.name&&s.lat&&s.lon).map((s,idx)=>({...s,id:s.id??idx,zone:s.zone??'centro'}));
      stops.forEach(s=>{
        ...
        const html=`
          <div>
            <b>${s.name}</b>
            <span class="popup-star ${starClass}" data-id="${s.id}" title="Aggiungi/rimuovi dai preferiti">⭐</span>
            <br>
            <div class="links">
    ...
    .catch(e=>{ console.error(e); alert('Errore nel caricamento delle fermate: '+e.message); });
  ...
  function renderFavoritesList(){
    ...
    Object.keys(zones).forEach(zoneKey=>{
      ...
      stopsInZone.forEach(stop=>{
        ...
        a.href=stop.url; a.target='_blank'; a.textContent=stop.name;
        ...
        btn.addEventListener('click',()=>{
          const idx=favorites.indexOf(stop.id.toString()); if(idx>-1) favorites.splice(idx,1);
          localStorage.setItem('favorites',JSON.stringify(favorites)); renderFavoritesList();
        });
  ...
  input.addEventListener('input',()=>{
    const q=normalize(input.value.trim()); suggestions.innerHTML='';
    if(!q){ markerCluster.clearLayers(); markers.forEach(m=>markerCluster.addLayer(m)); return; }
    ...
    matched.slice(0,10).forEach(m=>{
      ...
      div.addEventListener('click',()=>{ map.setView(m.getLatLng(),17); m.openPopup(); suggestions.innerHTML=''; });
      suggestions.appendChild(div);
    });
    markerCluster.clearLayers(); matched.forEach(m=>markerCluster.addLayer(m));
  ...
  locateBtn.addEventListener('click',()=>{
    ...
    navigator.geolocation.getCurrentPosition(
      ({coords})=>{
        locateBtn.classList.add('active'); const {latitude:latU,longitude:lonU}=coords;
